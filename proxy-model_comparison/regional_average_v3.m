...
for nmask=1:3
    ...
    if nmask~=2
    h=plot(m,MH_max(idx),'o','DisplayName','Reconstructed maximums');
    hold on
    end
    for i=1:length(modelList)
    ...
    hold off
    p=p+1;
    legend(h,'Fontsize',6,'Color','none')
...
function add_panel_label(ax, letter)
    text(ax, 0.02, 0.98, letter, ...
         ...
         'BackgroundColor', 'none', ...     % optional: makes it readable on dark plots
         'EdgeColor', 'none', ...           % optional: thin black border, or no border
         'Margin', 2);
end
%% visualize masks
figure;
